...
```json
{
  "primaryTaskId": "REGE-001",
  "parallelTracks": ["REGE-002", "REGE-003"],
  "dependencyGraph": {
    "REGE-001": [],
    "REGE-002": [],
    "REGE-003": ["REGE-002"]
...
```bash
node scripts/sprint-board.mjs update --id REGE-001 --state "In Progress"
node scripts/sprint-board.mjs journal --task REGE-001 --summary "what changed"
```
...
4. When marking a task `Failed`, record a pitfall entry alongside the state update:
   ```bash
   node scripts/sprint-board.mjs update --id REGE-001 --state "Failed" \
     --failure-type <gate|acceptance|review> --attempted "..." --hypothesis "..."
   node scripts/sprint-board.mjs pitfall --task REGE-001 \
     --failure-type <gate|acceptance|review> --attempted "..." --hypothesis "..." \
     [--missing-context "..."]
   ```
